...
export default function CardOrderConfirmation() {
  ...
  const handleFinalSubmit = () => {
    ...
    localStorage.setItem("pending_card_request", JSON.stringify(cardRequest));
    setIsSuccess(true);
  };

  if (isSuccess) {
    return (
      <div className="bper-confirmation-screen" style={{display:'flex', alignItems:'center', justifyContent:'center', padding:'20px'}}>
        <div className="info-group" style={{textAlign:'center', maxWidth:'450px', border:'none', boxShadow:'0 10px 25px rgba(0,0,0,0.1)'}}>
          ...
          <h2 style={{color:'#0f172a', fontWeight:'800', fontSize:'22px', marginBottom:'12px'}}>Demande envoyée avec succès !</h2>
          <p style={{color:'#64748b', fontSize:'14px', lineHeight:'1.6', marginBottom:'24px'}}>
            Vous recevrez un message de validation de la demande après les investigations des conditions et les règles de sécurité BPER Banca.
          </p>
  ...
  return (
    <div className="bper-confirmation-screen">
          ...
          <div className="main-grid">
            <aside className="visual-sidebar">
              <div className="sticky-wrapper">
                ...
                <div className="summary-card-account">
                  ...
                  <div className="summary-row"><span>Compte N°</span><span className="bold">{account.accountNumber}</span></div>
                  <div className="summary-row"><span>IBAN</span><span className="bold mono">{account.iban}</span></div>
                </div>
              </div>
            </aside>
            ...
            <section className="form-section">
              <div className="info-group">
                <div className="group-header"><User size={18} /><h3>Détails du titulaire</h3></div>
                <div className="info-grid">
                  <div className="info-item"><label>Nom et Prénom</label><p>{user.civilite} {user.nom} {user.prenom}</p></div>
                  <div className="info-item"><label>E-mail associé</label><p>{user.email}</p></div>
                  <div className="info-item"><label>Téléphone</label><p>{user.telephone}</p></div>
                </div>
              </div>

              <div className="info-group">
                <div className="group-header"><MapPin size={18} /><h3>Adresse d'expédition</h3></div>
                <div className="address-box">
                  <p className="main-address">{user.adresse}</p>
                  <p className="sub-address">{user.codePostal}, {user.ville} - {user.pays}</p>
                  <div className="delivery-notice"><Info size={14} /><span>Livraison par courrier recommandé sous 3 à 5 jours.</span></div>
                </div>
              </div>

              <div className="info-group">
                <div className="group-header"><MessageSquare size={18} /><h3>Note supplémentaire</h3></div>
                <textarea className="bper-input-text" placeholder="Informations utiles..." value={comment} onChange={(e) => setComment(e.target.value)} rows="4" />
              </div>

              <div className="confirmation-footer">
                <p className="legal-box">En confirmant, vous autorisez BPER Banca à émettre la carte.</p>
      ...
      <style jsx>{`
        ...
        @keyframes cardFloat { 0% { transform: rotateY(-5deg) rotateX(4deg) translateY(0); } 50% { transform: rotateY(5deg) rotateX(-4deg) translateY(-8px); } 100% { transform: rotateY(-5deg) rotateX(4deg) translateY(0); } }
        .card-physical-container { padding: 30px; background: #f8fafc; border-radius: 24px; display: flex; justify-content: center; box-shadow: 0 10px 25px rgba(0,0,0,0.02); }
        ...
        .btn-submit-order { width: 100%; background: #005a64; color: white; border: none; padding: 18px; border-radius: 12px; font-weight: 800; font-size: 16px; display: flex; justify-content: center; align-items: center; gap: 10px; cursor: pointer; box-shadow: 0 10px 15px rgba(0, 90, 100, 0.1); transition: 0.2s; }
        .btn-submit-order:hover { background: #00454d; }
        ...
        @media (max-width: 1000px) { .main-grid { grid-template-columns: 1fr; gap: 20px; } .sticky-wrapper { position: static; } .info-grid { grid-template-columns: 1fr; } .card-body { width: 100%; max-width: 310px; margin: 0 auto; } .visual-sidebar { order: -1; } .content-header h1 { font-size: 24px; } }
      `}</style>
    </div>
  );
}